...
class BaseYOLONeck_BICM(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.reduce_layers_bic = nn.ModuleList()
        for idx in range(len(in_channels) - 1):
            self.reduce_layers_bic.append(self.build_reduce_layer_bic(idx))
    ...
    def build_reduce_layer(self, idx: int):
        ...
        pass
    @abstractmethod
    def build_reduce_layer_bic(self, idx: int):
        """build reduce layer."""
        pass
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        stage0_bic = self.reduce_layers_bic[0](stage0)
        stage1_bic = self.reduce_layers_bic[1](inputs[0])
        reduce_outs = []
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            if self.upsample_feats_cat_first:
                if idx == 2:
                    top_down_layer_inputs = torch.cat([upsample_feat, feat_low, stage1_bic], 1)
                if idx ==1:
                    top_down_layer_inputs = torch.cat([upsample_feat, feat_low, stage0_bic], 1)
            else:
                top_down_layer_inputs = torch.cat([feat_low, upsample_feat, stage1_bic], 1)
            inner_out = self.top_down_layers[len(self.in_channels) - 1 - idx](
                top_down_layer_inputs)
            inner_outs.insert(0, inner_out)
        inner_outs[0] = self.top_down_layers[2](inner_outs[0])
        # bottom-up path
        outs = [inner_outs[0]]
