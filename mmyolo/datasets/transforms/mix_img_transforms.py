...
augmentations = A.Compose([
    A.RandomBrightnessContrast(brightness_limit=0.2, contrast_limit=0.1, p=0.01),  # 应用亮度/对比度调整的概率为 0.5
    A.RGBShift(p = 0.01),
    A.CLAHE(clip_limit=2.0, tile_grid_size=(2, 2), p=0.01),
    A.RandomGamma(gamma_limit=(80, 120), p=0.01),  # 应用Gamma校正的概率为 0.5
    # A.CLAHE(clip_limit=2.0, tile_grid_size=(2, 2), p=1)
])
...
@TRANSFORMS.register_module()
class AIGC(BaseMixImageTransform):

    def __init__(self,data_type):
        super().__init__()
        self.cropp_aigc_img = []
        self.cropp_aigc_img_class = []
        self.data_type = 'possion'
        shengcheng_path = 'shengcheng'
        for class_folder in os.listdir(shengcheng_path):
            for img_file in os.listdir(os.path.join(shengcheng_path, class_folder)):
                if img_file.endswith(".png"):
                    pass
                else:
                    continue
                img = cv2.imread(os.path.join(shengcheng_path, class_folder, img_file))
                self.cropp_aigc_img.append(copy.deepcopy(img))
                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 0 and random.random() < 0.2:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 1 and random.random() < 0.5:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 2 and random.random() < 0.2:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 3 and random.random() < 0.2:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 4 and random.random() < 0.2:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
                if int(class_folder) == 5 and random.random() < 0.15:
                                self.cropp_aigc_img.append(copy.deepcopy(img))
                                self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))                 
        print(len(self.cropp_aigc_img))
        print("加载AIGC图像成功")
    ...
    def pro_copypaste_aigc(self,img_size1, img4, labels4, class4, mosaic_ignore_flags, mixup_number=12,
                                         change_rotate=True, change_size=True,
                                         new_scale=0, use_cache = False, max_cached_images=100,data_type = 'possion',bigger = False):
        import random
        import cv2
        import torch
        mixup_number = random.randint(1, 4)

        for i in range(mixup_number):

            index = random.choices(range(0, len(self.cropp_aigc_img)), k=1)[0]
            im = self.cropp_aigc_img[index].copy()
            class_name = self.cropp_aigc_img_class[index].copy()
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        img = results['img']
        mosaic_bboxes.append(gt_bboxes)
        mosaic_bboxes_labels.append(gt_bboxes_labels)
        mosaic_ignore_flags.append(gt_ignore_flags)
        mosaic_bboxes = mosaic_bboxes[0].cat(mosaic_bboxes, 0)
        mosaic_bboxes_labels = np.concatenate(mosaic_bboxes_labels, 0)
        mosaic_ignore_flags = np.concatenate(mosaic_ignore_flags, 0)
        img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags = self.pro_copypaste_aigc(img.shape,img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags,data_type = self.data_type)
...
@TRANSFORMS.register_module()
class Pre_Copy(BaseMixImageTransform):
    ...
    def pro_copypaste_possion(self,img_size1, img4, labels4, class4, mosaic_ignore_flags, mixup_number=12,
                                         change_rotate=True, change_size=True,
                                         new_scale=0.1, use_cache = False, max_cached_images=100,data_type = 'possion',bigger = False):
        import random
        import cv2
        import torch
        cropp_img = []
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        mosaic_bboxes_labels = np.concatenate(mosaic_bboxes_labels, 0)
        mosaic_ignore_flags = np.concatenate(mosaic_ignore_flags, 0)
        img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags = self.pro_copypaste_possion(img.shape,img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags,data_type = self.data_type)
        img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags = self.pro_copypaste_aigc(img.shape,img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags,data_type = self.data_type)
...
@TRANSFORMS.register_module()
class CopyPaste_zaolei(BaseMixImageTransform):
    """Mosaic augmentation.
    ...
    def __init__(self,
        ...
        #         self.cropp_aigc_img.append(copy.deepcopy(img))
        #         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 0 and random.random() < 0.4:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 1 and random.random() < 0.9:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 2 and random.random() < 0.4:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 3 and random.random() < 0.4:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 4 and random.random() < 0.4:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))
        #         if int(class_folder) == 5 and random.random() < 0.1:
        #                         self.cropp_aigc_img.append(copy.deepcopy(img))
        #                         self.cropp_aigc_img_class.append(copy.deepcopy(np.array(int(class_folder))))                 
        #         print(len(self.cropp_aigc_img))
        #         print("加载AIGC图像成功")
    ...
    #                                      change_rotate=True, change_size=True,
    #                                      new_scale=0.1, max_cached_images=1000,data_type = 'possion', p_bigger = 0.1 ):
    #     import random
    #     import cv2
    #     import torch

    #     for label, label1 in zip(labels4, class4):
    #         if random.random() < p_bigger:
    #             beilv_p = random.uniform(0.05, 0.15)
    #         else:
    #             beilv_p = 0
    #         left = int(label.tensor.numpy()[0][0])
    #         lower = int(label.tensor.numpy()[0][1])
    ...
    #         # left = int(label.tensor.numpy()[0][0]) - x_p
    #         # lower = int(label.tensor.numpy()[0][1]) - y_p
    #         # right = int(label.tensor.numpy()[0][2]) + x_p
    #         # upper = int(label.tensor.numpy()[0][3]) + y_p
    #         class_name = label1
    #         if left == right or upper == lower:
    #             continue
    #         cropped = img4[lower - y_p:upper + y_p, left - x_p : right + x_p]  # (left, upper, right, lower)
    ...
    #     # if len(labels4.tensor.numpy()) <= 20:
    #     #     mixup_number = 2 * mixup_number
    #     mixup_number = random.randint(2, 12)
    ...
    #                 scale = 1
    #             else:
    #                 # my_list = [1, 1.1, 0.9, 0.95, 1.05]
    #                 # scale = random.choice(my_list)
    #                 scale = random.uniform(1- new_scale, 1 + new_scale)
    #             W = int(W * scale)
    #             H = int(H * scale)
    ...
    #             # x_p = int(x_p + abs(origin_W * scale - origin_W)/ 2)
    #             # y_p = int(y_p + abs(origin_H * scale - origin_H)/ 2)
    #             if H == 0 or W == 0:
    #                 continue
    #             im = cv2.resize(im, (W, H))
    ...
    #                 W = im.shape[1]
    #                 H = im.shape[0]
    #             # im = augmentations(image = im)['image']



    #                 # im = cv2.resize(im, None,fx=scale,fy=scale)
    #             point = [random.randint(150, img_size1[1] - W - 150), random.randint(150, img_size1[0] - H - 150)]
    #             if data_type == 'copy':
    #                 img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
    #             elif data_type == 'mixup':
    #                 r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
    ...
    #                     # im = (im * r + img4[point[1]:point[1] + H, point[0]:point[0] + W] * (1 - r)).astype(np.uint8)
    #                     img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
    #             # cv2.rectangle(img4, (point[0] + x_p, point[1] + y_p), (point[0] + W - x_p, point[1] + H - y_p), (0, 0, 0), 2)
    #             # cv2.putText(img4, str(classesname[class_name]), (point[0], point[1]), cv2.FONT_HERSHEY_SIMPLEX, 1,
    #             #             (0, 0, 0), 2)
    #             # 
    #             # cv2.namedWindow("Demo", cv2.WINDOW_NORMAL)
    #             # cv2.resizeWindow("Demo", 1280, 1280)
    ...
    def random_cache_mixup_single_mmyolo(self,img_size1, img4, labels4, class4, mosaic_ignore_flags, mixup_number=1,
                                         change_rotate=True, change_size=True,
                                         new_scale=0.1, max_cached_images=40,data_type = 'possion', p_bigger = 0):
        import random
        import cv2
        import torch
        for label, label1 in zip(labels4, class4):
        ...
        mixup_number = 1
        aigc_mixup_number = random.randint(1, 3)
        # for i in range(aigc_mixup_number):
        #     index = random.choices(range(0, len(self.cropp_aigc_img)), k=1)[0] 
        #     im = self.cropp_aigc_img[index].copy()
        #     class_name = self.cropp_aigc_img_class[index].copy()
        #     W = im.shape[1]
        #     H = im.shape[0]
        #     # if int(class_name) == 1:
        #     #     self.all_w = self.all_w + W
        #     #     self.all_h = self.all_h + H
        #     #     print(self.all_w)
        #     #     print(self.all_h)
        #     if change_size:
        #         scale = random.uniform(1 - new_scale, 1 + new_scale)
        ...
        #             W = im.shape[1]
        #             H = im.shape[0]
        #         point = [random.randint(W, img_size1[1] - W ), random.randint(300, img_size1[0] - H - 300)]
        #         if data_type == 'copy':
        #             img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
        #         elif data_type == 'mixup':
        #             r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
        ...
        for i in range(mixup_number):
            ...
            if len(im):
                ...
                if change_size:
                    ...
                    im = cv2.resize(im, (W, H))
                    # im = cv2.resize(im, None,fx=scale,fy=scale)
                point = [random.randint(150 + W, img_size1[1] - W - 150), random.randint(H + 150, img_size1[0] - H - 150)]
                if data_type == 'copy':
                    img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
                elif data_type == 'mixup':
                    r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
                    ...
                    try:
                        ...
                        # im = (im * r + img4[point[1]:point[1] + H, point[0]:point[0] + W] * (1 - r)).astype(np.uint8)
                        img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
                    # cv2.rectangle(img4, (point[0], point[1]), (point[0] + W, point[1] + H), (0, 0, 0), 2)
                    # cv2.putText(img4, str(classesname[class_name]), (point[0], point[1]), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    #             (0, 0, 0), 2)
                else:
                    print("None")
    ...
    # def pro_copypaste_aigc(self,img_size1, img4, labels4, class4, mosaic_ignore_flags, mixup_number=12,
    #                                      change_rotate=True, change_size=True,
    #                                      new_scale=0.1, use_cache = False, max_cached_images=100,data_type = 'copy',bigger = False):
    #     import random
    #     import cv2
    #     import torch
    ...
    #                 W = im.shape[1]
    #                 H = im.shape[0]
    #             point = [random.randint(300 + W, img_size1[1] - W - 300), random.randint(300, img_size1[0] - H - 300)]
    #             if data_type == 'copy':
    #                 img4[point[1]:point[1] + H, point[0]:point[0] + W] = im
    #             elif data_type == 'mixup':
    #                 r = np.random.beta(32.0, 32.0)  # mixup ratio, alpha=beta=32.0
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        # mosaic_img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags = self.pro_copypaste_aigc(mosaic_img.shape,mosaic_img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags)

        mosaic_img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags = self.random_cache_mixup_single_mmyolo(mosaic_img.shape,mosaic_img,mosaic_bboxes,mosaic_bboxes_labels,mosaic_ignore_flags, p_bigger = self.p_bigger)
