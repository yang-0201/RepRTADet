...
class CopyPasteIJCAI1(BaseTransform):
    def __init__(self, cache_num=1000, n=(-0.1, 0.15)):
        ...
        self.class_p = [0.22, 0.1, 0.16, 0.14, 0.12, 0.25]
        self.class_p = [0.23, 0.2, 0.18, 0.18, 0.18, 0.18]
        self.class_soft = True
    ...
    def transform(self,
        ...
        for i in range(len(gt_bboxes)):
            ...
            if self.class_soft:
                if random.random() < 3 * self.class_p[int(class_name)]:
                    self.cache_list.append([cropped.copy(), [left, lower, right, upper],
        ...
        # 2、判断是否满足开始贴图的条件
        if len(gt_bboxes_labels) <= 8:
            mixup_number = 2 + self.mix_number
        else:
...
class CopyPasteIJCAI(BaseTransform):
    def __init__(self, cache_num=1000, n=(-0.1, 0.15), p = 0.1):
        self.cache_num = cache_num
        self.cache_list = []

        self.class_p = [0.06, 0.22, 0.1, 0.14, 0.17, 0.05]
        self.class_p = [0.22, 0.05, 0.16, 0.14, 0.12, 0.25]
        ...
        if random.random() < self.p:
            ...
            for index in indexes:
                ...
                for i in range(5):
                    ...
                    # 过滤小目标
                    filter_indexes = torch.logical_and(temp_gt_bboxes.widths > 8, temp_gt_bboxes.heights > 8)
                    # print(filter_indexes)
                    temp_gt_bboxes = temp_gt_bboxes[filter_indexes]
                    copy_gt_bboxes_labels = copy_gt_bboxes_labels[filter_indexes.numpy()]
