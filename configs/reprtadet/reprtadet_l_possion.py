...
model_test_cfg = dict(
    ...
    nms=dict(type='soft_nms', iou_threshold=0.65),  # NMS type and threshold
...
dataset_zengguang = dict(
    type=dataset_type,
    metainfo=metainfo,
    data_root=data_root,
    ann_file=val_ann_file,
    data_prefix=dict(img=val_data_prefix),
    # ann_file='annotations/instances_zengguang100.json',
    # data_prefix=dict(img='images/zengguang100/'),
    filter_cfg=dict(filter_empty_gt=True, min_size=32),
    pipeline=train_pipeline)
...
train_dataloader = dict(
    ...
    # dataset = dict( type='ConcatDataset', datasets=[dataset1, dataset_zengguang]))
    dataset=dict(
        type=dataset_type,
        metainfo=metainfo,
        data_root=data_root,
...
load_from = 'rtmdet_l_syncbn_fast_8xb32-300e_coco_20230102_135928-ee3abdc4.pth'
train_cfg = dict(
...
# val and test switch
test_image_info = 'annotations/instances_test.json'
...
_test_evaluator = dict(
    ...
    ann_file=data_root + test_image_info,
    metric='bbox',
